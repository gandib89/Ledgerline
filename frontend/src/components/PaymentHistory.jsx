  ...
  return (
    <section className="report-surface payment-history">
      ...
      {history.isPending ? <AsyncState title="Loading payments" message="Checking receipt allocations." />
        : history.isError ? <AsyncState tone="error" title="Payment history unavailable" message={history.error.message} />
          : history.data.payments.length === 0 ? <AsyncState tone="empty" title="No payments allocated" message="The full invoice amount is still outstanding." />
